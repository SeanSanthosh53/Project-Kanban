...
"Peach": (253/255, 154/255, 162/255, 1)}
	...
	def add_card_dialog(self):
		...
		dialog = MDDialog(
				...
				type="simple",
				items=[AddCardDialogItem(text = i) for i in card_categories],
				buttons=[
	...
	def create_new_project(self):
		...
		new_project_name = self.add_project_dialog.children[0].children[2].children[0].ids.new_project_name.text
		if new_project_name not in self.data.keys():
			
			self.data[new_project_name] = { "Card Categories": {"Backlog": [], "In Progress": [], "Done": []}, "Due Date": "21/3/21", "WIP" : "0", "WTD": "0"}
			with open("data/UserData.json", "w") as file:
				json.dump(self.data, file, indent = 4)
			
			self.screen_order.append(new_project_name)
			
			self.add_container(new_project_name, "21/3/21", "0", "0")
		...
		toast("User tried to make a projet!")
	...
	def add_card(self, instance):
		today = date.today()
		self.new_card_due = list(map(str, [today.day, today.month, today.year]))
		...
		def create_card():
			...
			if title.strip() != "" and task.strip() != "":
				...
				pprint(self.data[self.root.current]["Card Categories"][category])
				with open("data/UserData.json", "w") as file:
					json.dump(self.data, file, indent = 4)
	...
	def add_card_due(self):
		...
		def get_date(due_date):
			self.new_card_due = [due_date.day, due_date.month, due_date.year]
			print(self.new_card_due)
	...
	def set_card_due(self, open_date_card):
		...
		def get_time(date):
			print(date)
			self.edit_card_due = '-'.join(list(map(str, [date.day, date.month, date.year])))
			open_date_card[0]['due date'] = [date.day, date.month, date.year]
			with open("data/UserData.json", "w") as file:
				json.dump(self.data, file, indent = 4)
	...
	def move_card(self, instance, card, category):
		print(f"Trying to move {card.title} from {category} to {instance}!")
		...
		with open("data/UserData.json", "w") as file:
			json.dump(self.data, file, indent = 4)
		pprint(self.data[self.root.current]["Card Categories"][instance])
	...
	def board_settings(self, instance):
		...
		def set_wip():
			...
			def set():
				wip_confirm.dismiss()
				self.data[open_project]["WIP"] = str(wip_num)
				with open("data/UserData.json", "w") as file:
					json.dump(self.data, file, indent = 4)
				self.refresh_main()
		...
		def rename_project(open_project):
			...
			def get_text():
				text = rename_dialog.children[0].children[2].children[0].ids.text_field.text
				if len(text) <= 17:
		...
		if instance.text == "Rename Project":
			...
			wip_dialog = MDDialog(
				title="Set WIP",
				type="custom",
				content_cls=Content(hint = "New 'Works in progress' limit", text = self.data[open_project]["WIP"]),
				buttons=[
					MDFlatButton(text="CANCEL", text_color=self.theme_cls.primary_color),
